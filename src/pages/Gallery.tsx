...
import { galleryItemsData } from '../data';
...
export default function Gallery() {
  ...
  // Filtering images
  const filteredGallery = galleryItemsData.filter(item => {
    if (activeCategory === 'all') return true;
    return item.category === activeCategory;
  });

  const openLightbox = (id: string) => {
    const idx = galleryItemsData.findIndex(item => item.id === id);
    if (idx !== -1) {
  ...
  const nextSlide = (e: React.MouseEvent) => {
    ...
    if (lightboxIndex !== null) {
      setLightboxIndex((prev) => (prev! + 1) % galleryItemsData.length);
    }
  };
  ...
  const prevSlide = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (lightboxIndex !== null) {
      setLightboxIndex((prev) => (prev! - 1 + galleryItemsData.length) % galleryItemsData.length);
    }
  };

  const activeLightboxItem = lightboxIndex !== null ? galleryItemsData[lightboxIndex] : null;
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent opacity-85 sm:opacity-0 group-hover:opacity-95 transition-all duration-300 flex flex-col justify-end p-5">
                  <div className="flex justify-between items-end">
                    <div>
                      <span className="text-[9px] font-mono font-black uppercase text-[#3B82F6] tracking-widest bg-[#3B82F6]/10 px-2.5 py-0.5 rounded-none border border-[#3B82F6]/20">
                        {item.category}
                      </span>
                      <h4 className="text-sm font-black uppercase text-white tracking-widest mt-2 max-w-[170px] truncate italic">
                        {item.title}
                      </h4>
                    </div>
                    {/* Visual indicators */}
                    <div className="rounded-none bg-[#3B82F6] p-2.5 text-white shadow-xl">
          ...
          >
            ...
            <div className="flex items-center justify-between text-zinc-400 max-w-7xl mx-auto w-full border-b border-white/10 pb-4">
              <div className="flex items-center space-x-3">
                <span className="rounded-none bg-[#3B82F6]/15 border border-[#3B82F6]/20 px-2.5 py-1 text-[9px] font-black uppercase text-[#3B82F6]">
                  {activeLightboxItem.category}
                </span>
                <p className="text-xs font-black text-white uppercase tracking-wider italic">{activeLightboxItem.title}</p>
              </div>
            ...
            <div className="text-center text-[10px] text-zinc-500 max-w-7xl mx-auto w-full flex flex-col sm:flex-row items-center justify-between border-t border-white/10 pt-4">
              <span className="font-mono uppercase tracking-widest">SHOT {lightboxIndex + 1} OF {galleryItemsData.length} IN MASTER DATABASE</span>
