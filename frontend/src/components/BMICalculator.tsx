...
export default function BMICalculator() {
  ...
  // States for imperial and unit systems
  const [weightLbs, setWeightLbs] = useState<number>(165);
  const [heightFt, setHeightFt] = useState<number>(5);
  const [heightIn, setHeightIn] = useState<number>(9);
  const [heightSystem, setHeightSystem] = useState<'metric' | 'imperial'>('metric');
  const [weightSystem, setWeightSystem] = useState<'metric' | 'imperial'>('metric');
  ...
  useEffect(() => {
    let computedBmi = 0;
    if (heightSystem === 'metric' && weightSystem === 'metric') {
      const heightInMeters = height / 100;
      if (heightInMeters > 0) {
        computedBmi = weight / (heightInMeters * heightInMeters);
      }
    } else {
      // Use imperial formula, converting values if necessary
      const heightInInches = heightSystem === 'imperial'
        ? (heightFt * 12) + heightIn
        : height / 2.54; // cm to inches
      const weightInLbs = weightSystem === 'imperial'
        ? weightLbs
        : weight * 2.20462; // kg to lbs
      if (heightInInches > 0) {
        computedBmi = (weightInLbs / (heightInInches * heightInInches)) * 703;
      }
    }
  ...
  }, [heightSystem, weightSystem, weight, height, weightLbs, heightFt, heightIn]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          {/* Unit Toggle Switches */}
            {/* Height Unit Toggle */}
            <div className="flex rounded-xl bg-zinc-950 p-1 border border-zinc-800/60 mb-4">
              <button
                onClick={() => setHeightSystem('metric')}
                className={`flex-1 rounded-lg py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${
                  heightSystem === 'metric' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                Height: Metric (CM)
              </button>
              <button
                onClick={() => setHeightSystem('imperial')}
                className={`flex-1 rounded-lg py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${
                  heightSystem === 'imperial' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
            ...
            <div className="flex rounded-xl bg-zinc-950 p-1 border border-zinc-800/60">
              <button
                ...
                className={`flex-1 rounded-lg py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${
                  weightSystem === 'metric' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                Weight: Metric (KG)
              </button>
              <button
                ...
                className={`flex-1 rounded-lg py-2.5 text-xs font-bold uppercase tracking-wider transition-colors ${
                  weightSystem === 'imperial' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                Weight: Imperial (LBS)
              </button>
          ...
          {weightSystem === 'metric' ? (
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-xs font-bold uppercase tracking-wider text-zinc-400">
                  Weight: <span className="text-white font-mono">{weightLbs} lbs</span>
                </label>
              </div>
              <input
                type="range"
          ...
          <div className={`rounded-2xl border ${borderColor} ${bgColor} p-6 h-full flex flex-col justify-between transition-all duration-300`}>
            ...
            <div className="space-y-4 pt-6 flex-grow">
              ...
              <div className="space-y-2">
                <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Recommended Fitex Gym Programs</div>
